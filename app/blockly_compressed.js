...
goog.dom.$dom=goog.dom.createDom;goog.dom.createElement=function(a){return goog.dom.createElement_(document,a)};
...
goog.ui.MenuItemRenderer.prototype.setEnableCheckBoxStructure=function(a,b,c){
	...
	c!=this.hasCheckBoxStructure(b)&&(
		...
		c?(c=this.getCompositeCssClass_(goog.ui.MenuItemRenderer.CompositeCssClassIndex_.CHECKBOX),	b.insertBefore(a.getDomHelper().createDom("DIV",c),b.firstChild||null)):b.removeChild(b.firstChild))
};
...
Blockly.VariableMap.prototype.createVariable = function(name,
  ...
  if (variable) {
    /*if (opt_id && variable.getId() != opt_id) { //ny
...
Blockly.Connection.prototype.connect_=function(a){
	var b=this,c=b.getSourceBlock(),d=a.getSourceBlock();a.isConnected()&&a.disconnect();
	if(b.isConnected()){
		var e=b.targetBlock(),f=b.getShadowDom();b.setShadowDom(null);
		if(e.isShadow())f=Blockly.Xml.blockToDom(e),e.dispose(),e=null;
		else if(b.type==Blockly.INPUT_VALUE){
			if(!e.outputConnection)throw"Orphan block does not have an output connection.";var g=Blockly.Connection.lastConnectionInRow_(d,e);g&&(e.outputConnection.connect(g),e=null)
		}else if(b.type==Blockly.NEXT_STATEMENT){
				if(!e.previousConnection)throw"Orphan block does not have a previous connection.";for(g=d;g.nextConnection;){var h=g.getNextBlock();if(h&&!h.isShadow())g=h;else{e.previousConnection.checkType_(g.nextConnection)&&(g.nextConnection.connect(e.previousConnection),e=null);break}}}
		if(e&&(b.disconnect(),Blockly.Events.recordUndo)){var k=Blockly.Events.getGroup();setTimeout(function(){e.workspace&&!e.getParent()&&(Blockly.Events.setGroup(k),e.outputConnection?e.outputConnection.bumpAwayFrom_(b):e.previousConnection&&e.previousConnection.bumpAwayFrom_(b),Blockly.Events.setGroup(!1))},Blockly.BUMP_DELAY)}b.setShadowDom(f)
	}
	var m;Blockly.Events.isEnabled()&&(m=new Blockly.Events.BlockMove(d));Blockly.Connection.connectReciprocally_(b,a);d.setParent(c);m&&(m.recordNew(),Blockly.Events.fire(m))
...
Blockly.Connection.prototype.disconnect=function(){
	var a=this.targetConnection;goog.asserts.assert(a,"Source connection not connected.");
	goog.asserts.assert(a.targetConnection==this,"Target connection not connected to source connection.");
	
	if(this.isSuperior()){
		var b=this.sourceBlock_;var c=a.getSourceBlock();a=this
...
Blockly.Gesture.prototype.updateIsDraggingFromFlyout_=function(){
	return this.targetBlock_.disabled?!1:!this.flyout_.isScrollable()||
	this.flyout_.isDragTowardWorkspace(this.currentDragDeltaXY_)?
		(this.startWorkspace_=this.flyout_.targetWorkspace_,this.startWorkspace_.updateScreenCalculationsIfScrolled(),Blockly.Events.getGroup()||Blockly.Events.setGroup(!0),this.startBlock_=null,
		this.targetBlock_=this.flyout_.createBlock(this.targetBlock_),
		this.targetBlock_.select(),!0):!1};
...
Blockly.Gesture.prototype.doFieldClick_=function(){this.startField_.showEditor_();this.bringBlockToFront_()};

Blockly.Gesture.prototype.doBlockClick_=function(){
	this.flyout_&&this.flyout_.autoClose?
		this.targetBlock_.disabled||(Blockly.Events.getGroup()||Blockly.Events.setGroup(!0),this.flyout_.createBlock(this.targetBlock_).scheduleSnapAndBump()):Blockly.Events.fire(new Blockly.Events.Ui(this.startBlock_,"click",void 0,void 0));
...
Blockly.VariableModel=function(a,b,c,d){this.workspace=a;this.name=b;this.type=c||"";this.id_=d||Blockly.utils.genUid();
Blockly.Events.fire(new Blockly.Events.VarCreate(this))};
Blockly.VariableModel.prototype.getId=function(){return this.id_};Blockly.VariableModel.compareByName=function(a,b){return goog.string.caseInsensitiveCompare(a.name,b.name)};Blockly.Variables={};Blockly.Variables.NAME_TYPE=Blockly.VARIABLE_CATEGORY_NAME;Blockly.Variables.allUsedVarModels=function(a){var b=a.getAllBlocks();a=Object.create(null);for(var c=0;c<b.length;c++){var d=b[c].getVarModels();if(d)for(var e=0;e<d.length;e++){var f=d[e];f.getId()&&(a[f.getId()]=f)}}b=[];for(var g in a)b.push(a[g]);return b};
...
Blockly.Variables.renameVariable=function(a,b,c){var d=function(e){
	var f= "Rename all "+b.type+" named '"+b.name+"' to:"// Blockly.Msg.RENAME_VARIABLE_TITLE.replace("%1",b.name); ny
	Blockly.Variables.promptName(f,e,function(e){if(e){var f=Blockly.Variables.nameUsedWithOtherType_(e,b.type,a);f?(f=Blockly.Msg.VARIABLE_ALREADY_EXISTS_FOR_ANOTHER_TYPE.replace("%1",e.toLowerCase()).replace("%2",f.type),Blockly.alert(f,function(){d(e)})):(a.renameVariableById(b.getId(),e),c&&c(e))}else c&&c(null)})};d("")};
Blockly.Variables.promptName=function(a,b,c){Blockly.prompt(a,b,function(a){a&&(a=a.replace(/[\s\xa0]+/g," ").replace(/^ | $/g,""),a==Blockly.Msg.RENAME_VARIABLE||a==Blockly.Msg.NEW_VARIABLE)&&(a=null);c(a)})};
...
Blockly.Xml.domToBlock = function(xmlBlock, workspace) {
  ...
  change_disconnted_color(topBlock) //ny
  return topBlock;
};
...
goog.global.Blockly||(goog.global.Blockly={});goog.global.Blockly.Xml||(goog.global.Blockly.Xml={});
...
Blockly.Block.prototype.allInputsFilled=function(a){void 0===a&&(a=!0);if(!a&&this.isShadow())return!1;for(var b=0,c;c=this.inputList[b];b++)if(c.connection&&(c=c.connection.targetBlock(),!c||!c.allInputsFilled(a)))return!1;return(b=this.getNextBlock())?b.allInputsFilled(a):!0
};
...
Blockly.BlockDragSurfaceSvg.prototype.SVG_=null;Blockly.BlockDragSurfaceSvg.prototype.dragGroup_=null;Blockly.BlockDragSurfaceSvg.prototype.container_=null;Blockly.BlockDragSurfaceSvg.prototype.scale_=1;Blockly.BlockDragSurfaceSvg.prototype.surfaceXY_=null;
...
Blockly.BlockSvg.prototype.renderDrawBottom_ = function(steps,
  ...
  if (this.nextConnection) {
    ...
    this.height += 1.8;  // Height of tab. // ny
  }
...
goog.getMsg=function(a,b){
	var c=goog.getMsg.blocklyMsgMap[a];c&&(a=Blockly.Msg[c]);
...
Blockly.FieldDropdown=function(a,d,b){ // ny
...
goog.inherits(Blockly.FieldDropdown,Blockly.Field);Blockly.FieldDropdown.fromJson=function(a){return new Blockly.FieldDropdown(a.options)};Blockly.FieldDropdown.CHECKMARK_OVERHANG=25;Blockly.FieldDropdown.MAX_MENU_HEIGHT=300;Blockly.FieldDropdown.ARROW_CHAR=goog.userAgent.ANDROID?"\u25bc":"\u25be";
...
Blockly.FieldDropdown.prototype.createMenu_=function(){
	var a=new goog.ui.Menu;a.setRightToLeft(this.sourceBlock_.RTL);
	for(var b=this.getOptions(),c=0;c<b.length;c++){
		var d=b[c][0],e=b[c][1];
		if("object"==typeof d){
			var f=new Image(d.width,d.height);
			f.src=d.src;f.alt=d.alt||"";d=f
		}
...
b;this.value_=a;this.setText(b.name)};
...
Blockly.FieldVariable.dropdownCreate=function(){
	...
	var defaultType = this.defaultType_
	if(!this.variable_)throw Error("Tried to call dropdownCreate on a variable field with no variable selected.");var a=this.getText(),b=null;this.sourceBlock_&&(b=this.sourceBlock_.workspace);var c=[];
	if(b)
		for(var d=this.getVariableTypes_(),e=0;e<d.length;e++){
			var f=b.getVariablesOfType(d[e]);
			if( f.length > 0 ){
				var tyep_selectedBlock = f["0"].type
				if(tyep_selectedBlock === defaultType)
					c=c.concat(f)
			}
		}
 ...
 // ny
	if(defaultType != "timer" && defaultType !="after"){
		b.push(["Rename "+defaultType+a,Blockly.RENAME_VARIABLE_ID]);
		b.push(["New "+defaultType+"" ,"CREATE_VARIABLE_BLOCK"]); // ny
...
Blockly.FieldVariable.prototype.onItemSelected=function(a,b){
	...
	if(this.sourceBlock_&&this.sourceBlock_.workspace){
		...
		if(c==Blockly.RENAME_VARIABLE_ID){Blockly.Variables.renameVariable(d,this.variable_);return}
		if(c==Blockly.DELETE_VARIABLE_ID){d.deleteVariableById(this.variable_.getId());return}
...
Blockly.Generator.prototype.blockToCode=function(a){
	if(!a)return"";if(a.disabled)return this.blockToCode(a.getNextBlock());var b=this[a.type];goog.asserts.assertFunction(b,'Language "%s" does not know how to generate code for block type "%s".',this.name_,a.type);
...
Blockly.Generator.prototype.statementToCode = function(block, name) {
  ...
  if (code) {
    //code = this.prefixLines(/** @type {string} */ (code), this.INDENT); ny
	return code;
  }
  return code;
};
...
Blockly.Names.prototype.getDistinctName=function(a,b){for(var c=this.safeName_(a),d="";this.dbReverse_[c+d]||c+d in this.reservedDict_;)d=d?d+1:2;c+=d;this.dbReverse_[c]=!0;return(b==Blockly.Variables.NAME_TYPE||b==Blockly.Names.DEVELOPER_VARIABLE_TYPE?this.variablePrefix_:"")+c};

Blockly.Names.prototype.safeName_=function(a){
	a?(a=encodeURI(a.replace(/ /g,"_")).replace(/[^\w]/g,"_"),-1!="0123456789".indexOf(a[0])&&(a=a)):a="unnamed";return a
};
...
Blockly.Flyout.prototype.createBlock=function(a){
	var b=null;Blockly.Events.disable();var c=this.targetWorkspace_.getAllVariables();this.targetWorkspace_.setResizesEnabled(!1);
	try{b=this.placeNewBlock_(a),Blockly.hideChaff()}
	finally{Blockly.Events.enable()}
...
Blockly.Flyout.prototype.placeNewBlock_=function(a){
	...
	var c=Blockly.Xml.blockToDom(a);b.setResizesEnabled(!1);c=Blockly.Xml.domToBlock(c,b);
	if(!c.getSvgRoot())throw"block is not rendered.";
	var d=b.getOriginOffsetInPixels(),e=this.workspace_.getOriginOffsetInPixels();a=a.getRelativeToSurfaceXY().scale(this.workspace_.scale);a=goog.math.Coordinate.sum(e,a);b=goog.math.Coordinate.difference(a,d).scale(1/b.scale);c.moveBy(b.x,b.y);
	change_disconnted_color(c)//ny connection
	return c
};
...
Blockly.Toolbox.prototype.clearSelection=function(){this.tree_.setSelectedItem(null)};Blockly.Toolbox.prototype.addStyle=function(a){Blockly.utils.addClass(this.HtmlDiv,a)};
...
Blockly.Toolbox.TreeControl.prototype.setSelectedItem=function(a){
	...
	if(a!=this.selectedItem_&&a!=b.tree_){
		...
		if(a){
			...
			a.getRowElement().style.backgroundColor=c;b.addColour_(a)
		}
