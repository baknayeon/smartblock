...
goog.events.fireListeners_=function(a,b,c,d){var e=!0;if(a=goog.events.getListenerMap_(a))if(b=a.listeners[b.toString()])for(b=b.concat(),a=0;a<b.length;a++){var f=b[a];f&&f.capture==c&&!f.removed&&(f=goog.events.fireListener(f,d),e=e&&!1!==f)}return e};

goog.events.fireListener=function(a,b){
	var c=a.listener,d=a.handler||a.src;a.callOnce&&goog.events.unlistenByKey(a);
	return c.call(d,b)
};

goog.events.getTotalListenerCount=function(){return goog.events.listenerCountEstimate_};goog.events.dispatchEvent=function(a,b){goog.asserts.assert(goog.events.Listenable.isImplementedBy(a),"Can not use goog.events.dispatchEvent with non-goog.events.Listenable instance.");return a.dispatchEvent(b)};
...
goog.events.markIeEvent_=function(a){var b=!1;if(0==a.keyCode)try{a.keyCode=-1;return}catch(c){b=!0}if(b||void 0==a.returnValue)a.returnValue=!0};goog.events.isMarkedIeEvent_=function(a){return 0>a.keyCode||void 0!=a.returnValue};
...
goog.events.EventTarget.dispatchEventInternal_=function(a,b,c){
	...
	if(c)
		for(var f=c.length-1;!b.propagationStopped_&&0<=f;f--){
			var g=b.currentTarget=c[f];e=g.fireListeners(d,!0,b)&&e
		}
...
goog.dom.createDom_=function(a,b){
	...
	if(!goog.dom.BrowserFeature.CAN_ADD_NAME_OR_TYPE_ATTRIBUTES&&d&&(d.name||d.type)){
		c=["<",c];d.name&&c.push(' name="',goog.string.htmlEscape(d.name),'"');
		if(d.type){
...
goog.ui.Control.prototype.handleMouseDown=function(a){
	this.isEnabled()
	&&(this.isAutoState(goog.ui.Component.State.HOVER)
		&&this.setHighlighted(!0),a.isMouseActionButton()
		&&(this.isAutoState(goog.ui.Component.State.ACTIVE)
		&&this.setActive(!0),this.renderer_
...
Blockly.VariableMap.prototype.createVariable=function(a,b,c){
	...
	if(c&&this.getVariableById(c))throw Error('Variable id, "'+c+'", is already in use.');c=c||Blockly.utils.genUid();
	b=b||"";
...
Blockly.VariableMap.prototype.getVariableById=function(a){for(var b=Object.keys(this.variableMap_),c=0;c<b.length;c++)for(var d=b[c],e=0,f;f=this.variableMap_[d][e];e++)if(f.getId()==a)return f;return null};

Blockly.VariableMap.prototype.getVariablesOfType=function(a){return(a=this.variableMap_[a||""])?a.slice():[]};Blockly.VariableMap.prototype.getVariableTypes=function(){return Object.keys(this.variableMap_)};
...
Blockly.Workspace.prototype.dispose=function(){this.listeners_.length=0;this.clear();delete Blockly.Workspace.WorkspaceDB_[this.id]};Blockly.Workspace.SCAN_ANGLE=3;
...
Blockly.Workspace.prototype.renameVariableInternal_=function(a,b){
	...
	if(c&&c.name!=b)var d=c.name;Blockly.Events.setGroup(!0);
	c=this.getAllBlocks();
...
Blockly.Workspace.prototype.renameVariableById=function(a,b){var c=this.getVariableById(a);this.renameVariableInternal_(c,b)};Blockly.Workspace.prototype.createVariable=function(a,b,c){return this.variableMap_.createVariable(a,b,c)};
...
Blockly.Workspace.prototype.undo=function(a){var b=a?this.redoStack_:this.undoStack_,c=a?this.undoStack_:this.redoStack_,d=b.pop();if(d){for(var e=[d];b.length&&d.group&&d.group==b[b.length-1].group;)e.push(b.pop());for(b=0;d=e[b];b++)c.push(d);e=Blockly.Events.filter(e,a);Blockly.Events.recordUndo=!1;try{for(b=0;d=e[b];b++)d.run(a)}finally{Blockly.Events.recordUndo=!0}}};
...
Blockly.Workspace.prototype.getBlockById=function(a){return this.blockDB_[a]||null};Blockly.Workspace.prototype.allInputsFilled=function(a){for(var b=this.getTopBlocks(!1),c=0,d;d=b[c];c++)if(!d.allInputsFilled(a))return!1;return!0};

Blockly.Workspace.prototype.getVariablesOfType=function(a){return this.variableMap_.getVariablesOfType(a)};Blockly.Workspace.prototype.getVariableTypes=function(){return this.variableMap_.getVariableTypes()};

Blockly.Workspace.prototype.getAllVariables=function(){return this.variableMap_.getAllVariables()};
Blockly.Workspace.WorkspaceDB_=Object.create(null);Blockly.Workspace.getById=function(a){return Blockly.Workspace.WorkspaceDB_[a]||null};
...
Blockly.Connection.prototype.connect_=function(a){
	...
	if(e&&(b.disconnect(),Blockly.Events.recordUndo)){
		...
		,Blockly.BUMP_DELAY)}b.setShadowDom(f)
	}
	var n;
...
Blockly.Tooltip.onMouseMove_=function(a){
	if(Blockly.Tooltip.element_&&Blockly.Tooltip.element_.tooltip&&!Blockly.WidgetDiv.isVisible()&&!Blockly.Tooltip.blocked_)
		if(Blockly.Tooltip.visible){
			var b=Blockly.Tooltip.lastX_-a.pageX;a=Blockly.Tooltip.lastY_-a.pageY;
...
Blockly.Gesture.prototype.cancel=function(){this.isEnding_||(Blockly.longStop_(),this.isDraggingBlock_?this.blockDragger_.endBlockDrag(this.mostRecentEvent_,this.currentDragDeltaXY_):this.isDraggingWorkspace_&&this.workspaceDragger_.endDrag(this.currentDragDeltaXY_),this.dispose())};
...
Blockly.Gesture.prototype.doBlockClick_=function(){
	this.flyout_&&this.flyout_.autoClose?this.targetBlock_.disabled
	||(Blockly.Events.getGroup()||Blockly.Events.setGroup(!0),this.flyout_.createBlock(this.targetBlock_).scheduleSnapAndBump()):Blockly.Events.fire(new Blockly.Events.Ui(this.startBlock_,"click",void 0,void 0));
...
Blockly.Field.getCachedWidth=function(a){var b=a.textContent+"\n"+a.className.baseVal,c;if(Blockly.Field.cacheWidths_&&(c=Blockly.Field.cacheWidths_[b]))return c;try{c=goog.userAgent.IE||goog.userAgent.EDGE?a.getBBox().width:a.getComputedTextLength()}catch(d){return 8*a.textContent.length}Blockly.Field.cacheWidths_&&(Blockly.Field.cacheWidths_[b]=c);return c};

Blockly.Field.startCache=function(){Blockly.Field.cacheReference_++;Blockly.Field.cacheWidths_||(Blockly.Field.cacheWidths_={})};
...
Blockly.Input.prototype.insertFieldAt=function(a,b,c){
	...
	if(!b&&!c)
		return a;goog.isString(b)&&(b=new Blockly.FieldLabel(b));
...
Blockly.Input.prototype.dispose=function(){//delete?
	for(var a=0,b;b=this.fieldRow[a];a++)
		b.dispose();
	this.connection&&this.connection.dispose();this.sourceBlock_=null
};
	
Blockly.ConnectionDB=function(){};Blockly.ConnectionDB.prototype=[];Blockly.ConnectionDB.constructor=Blockly.ConnectionDB;Blockly.ConnectionDB.prototype.addConnection=function(a){if(a.inDB_)throw"Connection already in database.";if(!a.getSourceBlock().isInFlyout){var b=this.findPositionForConnection_(a);this.splice(b,0,a);a.inDB_=!0}};
...
Blockly.Options.parseToolboxTree=function(a){
	a?
		("string"!=typeof a&&("undefined"==typeof XSLTProcessor&&a.outerHTML?a=a.outerHTML: a instanceof Element||(a=null)),"string"==typeof a&&(a=Blockly.Xml.textToDom(a)) ):
		a=null;
	return a
};
...
Blockly.Trashcan.prototype.BODY_HEIGHT_=25;
Blockly.Trashcan.prototype.LID_HEIGHT_=16;
Blockly.Trashcan.prototype.MARGIN_BOTTOM_=10;
...
Blockly.Trashcan.prototype.createDom=function(){
	...
	Blockly.utils.createSvgElement("image",{width:Blockly.SPRITE.width,x:-this.SPRITE_LEFT_,height:Blockly.SPRITE.height,y:-this.SPRITE_TOP_,"clip-path":"url(#blocklyTrashBodyClipPath"+a+")"},this.svgGroup_)
	...
	this.svgLid_=Blockly.utils.createSvgElement("image",{width:Blockly.SPRITE.width,x:-this.SPRITE_LEFT_,height:Blockly.SPRITE.height,y:-this.SPRITE_TOP_,"clip-path":"url(#blocklyTrashLidClipPath"+a+")"},this.svgGroup_);
...
Blockly.Trashcan.prototype.init=function(a){
	this.bottom_=this.MARGIN_BOTTOM_+a;this.setOpen_(!1);
...
Blockly.WorkspaceAudio.prototype.lastSound_=null;Blockly.WorkspaceAudio.prototype.dispose=function(){this.SOUNDS_=this.parentWorkspace_=null};
Blockly.WorkspaceAudio.prototype.load=function(a,b){if(a.length){try{var c=new window.Audio}catch(h){return}for(var d,e=0;e<a.length;e++){var f=a[e],g=f.match(/\.(\w+)$/);if(g&&c.canPlayType("audio/"+g[1])){d=new window.Audio(f);break}}d&&d.play&&(this.SOUNDS_[b]=d)}};
...
Blockly.Xml.blockToDom=function(a,b){
	...
	for(var e;e=a.inputList[d];d++)
		for(var f=0,g;g=e.fieldRow[f];f++){
			...
			if(h instanceof Blockly.FieldVariable){
				//h=a.workspace.getVariable(h.getValue());
				//var h_id = h.getId();
				//if(h_id)
					//g.setAttribute("id",h_id);
				var h_variableTypes = h.variableTypes;
				if(h_variableTypes)
					g.setAttribute("variabletype",h_variableTypes);
			...
			c.appendChild(g);;
					...
					&&h.EDITABLE
					&&(
						g=goog.dom.createDom("field",null,h.getValue()),g.setAttribute("name",h.name),h instanceof Blockly.FieldVariable
						&&(h=a.workspace.getVariable(h.getValue()))
	...
	for(d=0;e=a.inputList[d];d++){
		var k;g=!0;e.type!=Blockly.DUMMY_INPUT&&(h=e.connection.targetBlock(),e.type==Blockly.INPUT_VALUE?k=goog.dom.createDom("value"):e.type==Blockly.NEXT_STATEMENT&&(k=goog.dom.createDom("statement")),f=e.connection.getShadowDom(),!f||h&&h.isShadow()||k.appendChild(Blockly.Xml.cloneShadow_(f)),h&&(k.appendChild(Blockly.Xml.blockToDom(h,b)),g=!1),k.setAttribute("name",e.name),g||c.appendChild(k))
	}
	a.inputsInlineDefault!=a.inputsInline&&c.setAttribute("inline",a.inputsInline);
	a.isCollapsed()&&c.setAttribute("collapsed",!0);
	a.disabled&&c.setAttribute("disabled",!0);a.isDeletable()||a.isShadow()||c.setAttribute("deletable",!1);a.isMovable()||a.isShadow()||c.setAttribute("movable",!1);
	...
	/*if(a instanceof Blockly.Workspace){
		var c=a;a=b;b=c;console.warn("Deprecated call to Blockly.Xml.domToBlock, swap the arguments.")
	}
	...
	}finally{Blockly.Events.enable()}Blockly.Events.isEnabled()&&Blockly.Events.fire(new Blockly.Events.BlockCreate(d));
	return d*/
};
	...
	for(var f=null,g=0,h;h=a.childNodes[g];g++)
		if(3!=h.nodeType){
			...
			switch(h.nodeName.toLowerCase()){
				case "mutation":c.domToMutation&&(c.domToMutation(h),c.initSvg&&c.initSvg());
				break;
				case "comment":
					c.setCommentText(h.textContent);
					var l=h.getAttribute("pinned");l&&!c.isInFlyout&&setTimeout(function(){c.comment&&c.comment.setVisible&&c.comment.setVisible("true"==l)},1);f=parseInt(h.getAttribute("w"),10);
					h=parseInt(h.getAttribute("h"),10);
					!isNaN(f)&&!isNaN(h)&&c.comment&&c.comment.setVisible&&c.comment.setBubbleSize(f,h);
				break;
				case "data":c.data=h.textContent;
				break;
				...
				case "field":
					...
					if(k instanceof Blockly.FieldVariable){
						...
						//if(null!=p && p!==m.type ) ny we donot need this
					...
					if(!k){console.warn("Ignoring non-existent field "+f+" in block "+d);break}
					k.setValue(n);
				...
				break;
				default:console.warn("Ignoring unknown tag: "+h.nodeName)}
		}
...
Blockly.Xml.deleteNext=function(a){for(var b=0,c;c=a.childNodes[b];b++)if("next"==c.nodeName.toLowerCase()){a.removeChild(c);break}};
...
this.grid_&&this.grid_.update(this.scale);this.recordDeleteAreas();return this.svgGroup_};
...
Blockly.WorkspaceSvg.prototype.addTrashcan_=function(a){this.trashcan=new Blockly.Trashcan(this);var b=this.trashcan.createDom();this.svgGroup_.insertBefore(b,this.svgBlockCanvas_);return this.trashcan.init(a)};
...
Blockly.WorkspaceSvg.prototype.updateToolbox=function(a){
	if(a=Blockly.Options.parseToolboxTree(a)){
		...
		if(a.getElementsByTagName("category").length){
			...
			if(!this.flyout_)throw"Existing toolbox has categories.  Can't change mode.";this.options.languageTree=a;
...
Blockly.WorkspaceSvg.prototype.getAudioManager=function(){return this.audioManager_};Blockly.WorkspaceSvg.prototype.getGrid=function(){return this.grid_};Blockly.WorkspaceSvg.prototype.setVisible=Blockly.WorkspaceSvg.prototype.setVisible;
...
Blockly.Mutator.reconnect=function(a,b,c){if(!a||!a.getSourceBlock().workspace)return!1;c=b.getInput(c).connection;var d=a.targetBlock();
return d&&d!=b||c.targetConnection==a?!1:(c.isConnected()&&c.disconnect(),c.connect(a),!0)};

goog.global.Blockly||(goog.global.Blockly={});goog.global.Blockly.Mutator||(goog.global.Blockly.Mutator={});
goog.global.Blockly.Mutator.reconnect=Blockly.Mutator.reconnect;Blockly.Warning=function(a){Blockly.Warning.superClass_.constructor.call(this,a);this.createIcon();this.text_={}};goog.inherits(Blockly.Warning,Blockly.Icon);Blockly.Warning.prototype.collapseHidden=!1;
...
Blockly.Block.prototype.data=null;Blockly.Block.prototype.colour_="#000000";Blockly.Block.prototype.hue_=null;
	...
	if(this.workspace){
		...
		try{
			...
			for(b=0;b<d.length;b++){
				var e=d[b];e.isConnected()&&e.disconnect();
				d[b].dispose()
			}
		}
		finally{
...
Blockly.Block.prototype.setColour=function(a){
	if(typeof a == "string"){//ny
		this.hue_=null,this.colour_=a;
	}
	else{
		var b=Number(a);
...
Blockly.Block.prototype.setOnChange=function(a){if(a&&!goog.isFunction(a))throw Error("onchange must be a function.");this.onchangeWrapper_&&this.workspace.removeChangeListener(this.onchangeWrapper_);if(this.onchange=a)this.onchangeWrapper_=a.bind(this),this.workspace.addChangeListener(this.onchangeWrapper_)};
...
Blockly.Block.prototype.getVars=function(){
	for(var a=[],b=0,c;c=this.inputList[b];b++)
		for(var d=0,e;e=c.fieldRow[d];d++)e 
			instanceof Blockly.FieldVariable&&a.push(e.getValue());
	return a
};
...
Blockly.Block.prototype.appendValueInput = function(name) {
  return this.appendInput_(Blockly.INPUT_VALUE, name, null);
};

Blockly.Block.prototype.appendStatementInput = function(name) {
  return this.appendInput_(Blockly.NEXT_STATEMENT, name, null);
};
Blockly.Block.prototype.appendDummyInput = function(opt_name) {
  return this.appendInput_(Blockly.DUMMY_INPUT, opt_name || '', null);
};

Blockly.Block.prototype.appendDummyInput_n_th = function(opt_name, num) {
  return this.appendInput_(Blockly.DUMMY_INPUT, opt_name || '', num);
};
Blockly.Block.prototype.appendValueInput_n_th = function(name, num) {
  return this.appendInput_(Blockly.INPUT_VALUE, name, num);
};
...
Blockly.Block.prototype.appendInput_ = function(type, name, num) {
  var connection = null;
...
Blockly.ContextMenu.blockDuplicateOption=function(a){
	var b=!0;a.getDescendants().length>a.workspace.remainingCapacity()&&(b=!1);
	return{text:Blockly.Msg.DUPLICATE_BLOCK,enabled:b,callback:function(){
		Blockly.duplicate_(a)
		}
	}
};

Blockly.ContextMenu.blockCommentOption=function(a){var b={enabled:!goog.userAgent.IE};a.comment?(b.text=Blockly.Msg.REMOVE_COMMENT,b.callback=function(){a.setCommentText(null)}):(b.text=Blockly.Msg.ADD_COMMENT,b.callback=function(){a.setCommentText("")});return b};
...
goog.inherits(Blockly.BlockSvg,Blockly.Block);Blockly.BlockSvg.prototype.height=0;Blockly.BlockSvg.prototype.width=0;Blockly.BlockSvg.prototype.dragStartXY_=null;Blockly.BlockSvg.prototype.warningTextDb_=null;Blockly.BlockSvg.INLINE=-1;
...
Blockly.BlockSvg.prototype.initSvg=function(){
	...
	this.updateColour();this.updateMovable();this.workspace.options.readOnly||this.eventsInit_||Blockly.bindEventWithChecks_(this.getSvgRoot(),"mousedown",this,this.onMouseDown_);this.eventsInit_=!0;this.getSvgRoot().parentNode||this.workspace.getCanvas().appendChild(this.getSvgRoot())
};
...
Blockly.BlockSvg.prototype.bringToFront=function(){var a=this;do{var b=a.getSvgRoot();b.parentNode.appendChild(b);a=a.getParent()}while(a)};
...
Blockly.BlockSvg.prototype.appendInput_ = function(type, name, num) {
  var input = Blockly.BlockSvg.superClass_.appendInput_.call(this, type, name, num);
	...
	if(this.outputConnection)
		...
		if(this.previousConnection){
			...
			c&&c.getNextBlock()==this&&(this.squareTopLeftCorner_=!0)
				...
				Blockly.BlockSvg.START_HAT&&(this.startHat_=this.squareTopLeftCorner_=!0,this.height+=Blockly.BlockSvg.START_HAT_HEIGHT,b.rightEdge=Math.max(b.rightEdge,100));
...
Blockly.BlockSvg.TAB_WIDTH+",-0.5 q "+-.19*Blockly.BlockSvg.TAB_WIDTH+",-5.5 0,-11"),b.push("m",.92*Blockly.BlockSvg.TAB_WIDTH+",1 V 0.5 H 1")),this.width+=Blockly.BlockSvg.TAB_WIDTH):this.RTL||(this.squareTopLeftCorner_?b.push("V",.5):b.push("V",Blockly.BlockSvg.CORNER_RADIUS));a.push("z")
	};

Blockly.Events={};Blockly.Events.group_="";Blockly.Events.recordUndo=!0;Blockly.Events.disabled_=0;Blockly.Events.CREATE="create";Blockly.Events.BLOCK_CREATE=Blockly.Events.CREATE;Blockly.Events.DELETE="delete";
Blockly.Events.BLOCK_DELETE=Blockly.Events.DELETE;Blockly.Events.CHANGE="change";Blockly.Events.BLOCK_CHANGE=Blockly.Events.CHANGE;Blockly.Events.MOVE="move";Blockly.Events.BLOCK_MOVE=Blockly.Events.MOVE;Blockly.Events.VAR_CREATE="var_create";Blockly.Events.VAR_DELETE="var_delete";
...
goog.inherits(Blockly.Events.Move,Blockly.Events.Abstract);Blockly.Events.BlockMove=Blockly.Events.Move;Blockly.Events.Move.prototype.type=Blockly.Events.MOVE;
...
Blockly.FieldDropdown=function(a,d,b){
	this.menuGenerator_=a;this.trimOptions_();
	if(d){	
		Blockly.FieldDropdown.superClass_.constructor.call(this,d,b)
	}else{	
...
goog.inherits(Blockly.FieldDropdown,Blockly.Field);Blockly.FieldDropdown.CHECKMARK_OVERHANG=25;Blockly.FieldDropdown.ARROW_CHAR=goog.userAgent.ANDROID?"\u25bc":"\u25be";Blockly.FieldDropdown.prototype.CURSOR="default";Blockly.FieldDropdown.prototype.value_="";Blockly.FieldDropdown.prototype.imageElement_=null;
...
Blockly.FieldDropdown.prototype.addTouchStartListener_=function(a){a.getHandler().listen(a.getElement(),goog.events.EventType.TOUCHSTART,function(a){this.getOwnerControl(a.target).handleMouseDown(a)})};
...
Blockly.FieldDropdown.prototype.createMenu_=function(){
	...
	for(var b=this.getOptions(),c=0;c<b.length;c++){
		var d=b[c][0],e=b[c][1];
		if("object"==typeof d){
			var f=new Image(d.width,d.height);f.src=d.src;f.alt=d.alt||"";d=f}
			d=new goog.ui.MenuItem(d);d.setRightToLeft(this.sourceBlock_.RTL);
			d.setValue(e);d.setCheckable(!0);a.addChild(d,!0);d.setChecked(e==this.value_)
	}
	return a
};
...
Blockly.Variables.allUsedVariables=function(a){
	if(a instanceof Blockly.Block)
		...
		b=a.getAllBlocks();
	else throw"Not Block or Workspace: "+a;a=Object.create(null);
...
Blockly.Variables.createVariable=function(a,b,c){
	var d=function(e){
		Blockly.Variables.promptName(
			Blockly.Msg.NEW_VARIABLE_TITLE,e,function(e){
				e?a.getVariable(e)?Blockly.alert(Blockly.Msg.VARIABLE_ALREADY_EXISTS.replace("%1",e.toLowerCase()),function(){d(e)}):(a.createVariable(e,c),b&&b(e)):b&&b(null)
				}
			)
		};
	d("")
...
Blockly.Variables.createVariableButtonHandler=function(a,b,c){
	var d=c||"Switch",
		e=function(c){
			Blockly.Variables.promptName(
				Blockly.Msg.NEW_VARIABLE_TITLE,c,function(c){
					if(c){
						var f=Blockly.Variables.nameUsedWithAnyType_(c,a);
						if(f){
							var g=c.toLowerCase();
							if(f.type==d)
								var n=Blockly.Msg.VARIABLE_ALREADY_EXISTS.replace("%1",g);
							else n=Blockly.Msg.VARIABLE_ALREADY_EXISTS_FOR_ANOTHER_TYPE,n=n.replace("%1",g).replace("%2",f.type);Blockly.alert(n,function(){e(c)})
						}
...
Blockly.FieldVariable.prototype.onItemSelected=function(a,menuItem){
	...
	if(this.sourceBlock_&&this.sourceBlock_.workspace){
		...
		if(e)
			...
			if(id==Blockly.DELETE_VARIABLE_ID){
				d.deleteVariable(this.getText());
				return
			}
			if(id=="CREATE_VARIABLE_BLOCK"){// ny new block
				var block=this.sourceBlock_
				var Blocktype = block.type
				var vartype = this.variableTypes
				deviceCount.up(vartype)
				var newBlock = d.newBlock(Blocktype, null);
				var b=Blockly.clipboardXml_;
				var c=Blockly.clipboardSource_;

				change_disconnted_color(newBlock);
				Blockly.copy_(newBlock);
				newBlock.workspace.paste(Blockly.clipboardXml_);
...
Blockly.Generator.prototype.workspaceToCode=function(a){
	...
	var b=[];
	this.init(a);a=a.getTopBlocks(!0);
...
Blockly.Generator.prototype.blockToCode=function(a){
	...
	if(goog.isArray(b)) return b; //ny
	else if(goog.isString(b)){
...
Blockly.Names.prototype.getDistinctName=function(a,b){for(var c=this.safeName_(a),d="";this.dbReverse_[c+d]||c+d in this.reservedDict_;)d=d?d+1:2;c+=d;this.dbReverse_[c]=!0;return(b==Blockly.Variables.NAME_TYPE?this.variablePrefix_:"")+c};
...
Blockly.Flyout.prototype.show=function(a){
	...
	for(var b=[],c=[],d=this.permanentlyDisabled_.length=0,e;e=a[d];d++)
		...
		var f=e.tagName.toUpperCase(),g=this.horizontalLayout_?this.GAP_X:this.GAP_Y;
			if("BLOCK"==f)
				f=Blockly.Xml.domToBlock(e,this.workspace_),f.disabled&&this.permanentlyDisabled_.push(f),b.push({type:"block",block:f}),e=parseInt(e.getAttribute("gap"),10),c.push(isNaN(e)?g:e);
				...
				e=new Blockly.FlyoutButton(this.workspace_,this.targetWorkspace_,e,"LABEL"==f),b.push({type:"button",button:e}),c.push(g)
		}
	this.layout_(b,c);
...
Blockly.Flyout.prototype.createBlock=function(a){
	var b=null;Blockly.Events.disable();
...
Blockly.HorizontalFlyout=function(a){a.getMetrics=this.getMetrics_.bind(this);a.setMetrics=this.setMetrics_.bind(this);Blockly.HorizontalFlyout.superClass_.constructor.call(this,a);this.horizontalLayout_=!0};goog.inherits(Blockly.HorizontalFlyout,Blockly.Flyout);
...
d.moveBy(a/this.workspace_.scale-this.MARGIN-Blockly.BlockSvg.TAB_WIDTH-e,0)),d.flyoutRect_&&this.moveRectToBlock_(d.flyoutRect_,d);if(this.RTL)for(c=0;d=this.buttons_[c];c++)b=d.getPosition().y,d.moveTo(a-d.width-this.MARGIN-Blockly.BlockSvg.TAB_WIDTH,b);this.width_=a;this.targetWorkspace_.resize()}};
...
Blockly.Toolbox.prototype.width=0;Blockly.Toolbox.prototype.height=0;Blockly.Toolbox.prototype.selectedOption_=null;Blockly.Toolbox.prototype.lastCategory_=null;
...
Blockly.Toolbox.prototype.init=function(){
	...
	b={
		disabledPatternId:a.options.disabledPatternId,parentWorkspace:a,RTL:a.RTL,oneBasedIndex:a.options.oneBasedIndex,horizontalLayout:a.horizontalLayout,toolboxPosition:a.options.toolboxPosition};this.flyout_=null;this.flyout_=a.horizontalLayout?new Blockly.HorizontalFlyout(b):new Blockly.VerticalFlyout(b);goog.dom.insertSiblingAfter(this.flyout_.createDom("svg"),this.workspace_.getParentSvg());this.flyout_.init(a);
		this.config_.cleardotPath=a.options.pathToMedia+"1x1.gif";this.config_.cssCollapsedFolderIcon="blocklyTreeIconClosed"+(a.RTL?"Rtl":"Ltr");
...
Blockly.Toolbox.prototype.dispose=function(){
	this.flyout_.dispose();this.tree_.dispose();
	goog.dom.removeNode(this.HtmlDiv);this.lastCategory_=this.workspace_=null
};
...
Blockly.Toolbox.prototype.addColour_ = function(opt_tree) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    if (element) {
      if (this.hasColours_) {
        var border = '13px solid ' + (child.hexColour || '#def5f8');//ny
      } else {
        var border = 'none';
      }
...
Blockly.Toolbox.prototype.clearSelection=function(){this.tree_.setSelectedItem(null)};Blockly.Toolbox.prototype.addDeleteStyle=function(){Blockly.utils.addClass(this.HtmlDiv,"blocklyToolboxDelete")};
...
Blockly.Toolbox.TreeControl.prototype.setSelectedItem=function(a){
	...
	if(a!=this.selectedItem_&&a!=b.tree_){
		...
		if(a){//ny
			var c="#def5f8"; //var c=a.hexColour||"#57e";
...
Blockly.Css.CONTENT = [
  ...
  */
  ...
  '.blocklyMainBackground {', // main workspace ny
    'stroke-width: 1;',
    'stroke: #fff;',  /* Equates to #ddd due to border being off-pixel. */
  '}',
  ...
  '.blocklyFlyoutBackground {', // selected toolbox 
    'fill: #fff;',
    'fill-opacity: .8;',
  '}',
  ...
  */
  ...
  '.blocklyWidgetDiv .goog-menu {',
    ...
    'border-width: 1px;',
    'cursor: default;',
    'font: normal 13px Arial, NotoSansCJKkr-Thin;',
    'margin: 0;',
    'outline: none;',
    'padding: 4px 0;',
    'position: absolute;',
...
Blockly.WidgetDiv={};Blockly.WidgetDiv.DIV=null;Blockly.WidgetDiv.owner_=null;Blockly.WidgetDiv.dispose_=null;Blockly.WidgetDiv.createDom=function(){Blockly.WidgetDiv.DIV||(Blockly.WidgetDiv.DIV=goog.dom.createDom("DIV","blocklyWidgetDiv"),document.body.appendChild(Blockly.WidgetDiv.DIV))};
...
Blockly.createMainWorkspace_=function(a,b,c,d){
	...
	e.translate(0,0);
	Blockly.mainWorkspace=e;b.readOnly||b.hasScrollbars||e.addChangeListener(function(){
		if(!e.isDragging()){
			...
			if(a.contentTop<d||a.contentTop+a.contentHeight>a.viewHeight+d||a.contentLeft<(b.RTL?a.viewLeft:c)||a.contentLeft+a.contentWidth>(b.RTL?a.viewWidth:a.viewWidth+c))
				for(var k=e.getTopBlocks(!1),n=0,p;p=k[n];n++){
					...
					0<q&&p.moveBy(q,0);
					l=c+a.viewWidth-25-l.x+(b.RTL?m.width:0);0>l&&p.moveBy(l,0)
				}
			}
		});
	Blockly.svgResize(e);
...
Blockly.bindEventWithChecks_=function(a,b,c,d,e,f){
	var g=!1,h=function(a){
		var b=!e;a=Blockly.Touch.splitEventByTouches(a);
...
Blockly.bindEvent_=function(a,b,c,d){
	...
	return f
};Blockly.unbindEvent_=function(a){for(;a.length;){var b=a.pop(),c=b[2];b[0].removeEventListener(b[1],c,!1)}return c};
